...
contract('Election', function (accounts) {
  ...
  it('allows a voter to cast a vote', function () {
    return Election.deployed().then(function (instance) {
      ...
      assert.equal(receipt.logs[0].args._candidateId.toNumber(), candidateId, 'the candidate id is eq');

      return electionInstance.voters(accounts[0]);
    }).then(function (voted) {
      assert(voted, 'the voter was marked');
  ...
  it('throws an exception for invalid candidates', function () {
    return Election.deployed().then(function (instance) {
      electionInstance = instance;
      var invalidCandidateId = 101;

      return electionInstance.vote(invalidCandidateId, {
        from: accounts[1]
      });
      ...
      assert.equal(voteCount, 1, 'candidate 1 did not receive any new votes');
